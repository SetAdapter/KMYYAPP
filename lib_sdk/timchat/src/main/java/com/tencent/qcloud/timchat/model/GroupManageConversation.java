...
public class GroupManageConversation extends Conversation {
    ...
    public GroupManageConversation(TIMGroupPendencyItem message){
        lastMessage = message;
        name = TimApplication.getContext().getString(R.string.conversation_system_group);
    }
    ...
    public String getLastMessageSummary() {
        ...
        switch (lastMessage.getPendencyType()){
            case INVITED_BY_OTHER:
                if (isSelf){
                    return TimApplication.getContext().getResources().getString(R.string.summary_me)+
                            TimApplication.getContext().getResources().getString(R.string.summary_group_invite)+
                            to+
                            TimApplication.getContext().getResources().getString(R.string.summary_group_add);
                }else{
                    if (to.equals(UserInfo.getInstance().getId())){
                        return from+
                                TimApplication.getContext().getResources().getString(R.string.summary_group_invite)+
                                TimApplication.getContext().getResources().getString(R.string.summary_me)+
                                TimApplication.getContext().getResources().getString(R.string.summary_group_add);
                    }else{
                        return from+
                                TimApplication.getContext().getResources().getString(R.string.summary_group_invite)+
            ...
            case APPLY_BY_SELF:
                if (isSelf){
                    return TimApplication.getContext().getResources().getString(R.string.summary_me)+
                            TimApplication.getContext().getResources().getString(R.string.summary_group_apply)+
                            GroupInfo.getInstance().getGroupName(lastMessage.getGroupId());
                }else{
                    return from+ TimApplication.getContext().getResources().getString(R.string.summary_group_apply)+GroupInfo.getInstance().getGroupName(lastMessage.getGroupId());
                }
