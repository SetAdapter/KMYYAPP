...
public class BuyVVConsultActivity extends BaseActivity implements View.OnClickListener {
...
//    public static final int SELECT_USER_MEMBER_REQ = 10001;
    ...
    /*public static void startBuyVVConsultActivity(Context context, String doctorId, int type, String money) {
        Intent intent = new Intent(context, BuyVVConsultActivity.class);
    ...
    public static void startBuyVVConsultActivity(Context context, String doctorId, int type, String money, DoctorScheduleItem item) {
        Intent intent = new Intent(context, BuyVVConsultActivity.class);
        intent.putExtra(DOCTOR_INFO, doctorId);
        intent.putExtra(BUY_TYPE, type);
        intent.putExtra(MONEY, money);
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mConsultType = getIntent().getIntExtra(BUY_TYPE, 0);
        mDoctorID = getIntent().getStringExtra(DOCTOR_INFO);
        scheduleItem = (DoctorScheduleItem) getIntent().getSerializableExtra(TIME);
        if (scheduleItem != null){
            mTimeInfo = scheduleItem;
            mTime.setText(scheduleItem.mDateStr+" "+scheduleItem.mStartTime+"~"+scheduleItem.mEndTime);
        }
        if (mConsultType == 0) {
            setBarTitle(getString(R.string.appointment_voice_consult_title));
            mOPDType = HttpUserOPDRegisters.OPDTYPE_IM;
            getUserPackages(2);
        } else if (mConsultType == 1) {
            setBarTitle(getString(R.string.appointment_video_consult_title));
            mOPDType = HttpUserOPDRegisters.OPDTYPE_VIDEO;
            getUserPackages(3);
        }
        mMoney = getIntent().getStringExtra(MONEY);
        mMoneyValueTxt.setText(mMoney);
        mServiceMoneyTxt.setText(getString(R.string.service_account)+mMoney);
    ...
    private void buyConsultService() {
        ...
        if (TextUtils.isEmpty(mTime.getText().toString())){
            ToastUtils.show(this, getString(R.string.plz_choose_time_of_see_doctor), Toast.LENGTH_SHORT);
            return;
        }
...
//                uploadImageClient.start();
    ...
    private void createRecordAndPay(List<HttpUserConsults.ImageFile> imageFiles, int type) {
        addDoctorSchedule(imageFiles,type);
    }

    private void showCreateConsultDialog() {
        mOnCreateConsultDialog = new ProgressDialog(this);
        mOnCreateConsultDialog.setMessage(getString(R.string.create_consult_history));
...
//        CommonUtils.startActivity(this, SelectUserMemberActivity.class);
    ...
    public void setMainPayPosition(EventApi.SelectTime time) {
        mTimeInfo = time.doctorScheduleItem;
        mTime.setText(mTimeInfo.mDateStr+" "+mTimeInfo.mStartTime+"~"+mTimeInfo.mEndTime);
    }
    ...
    private AdapterView.OnItemClickListener mItemClick = new AdapterView.OnItemClickListener() {
        ...
        public void onItemClick(AdapterView<?> parent, View view, int position,
            ...
            MultiImageSelector.create(BuyVVConsultActivity.this)
                    .showCamera(true)
                    .count(3)
                    .multi()
                    .origin(mSelectPath)
                    .start(BuyVVConsultActivity.this, REQUEST_IMAGE);
    ...
    private void getMemberDefault() {//获取默认成员
        ...
        new HttpClient(BuyVVConsultActivity.this, event).start();
    }

    private void selectTime(){
        BuyVideoConsultActivity.startBuyVideoConsultActivity(this,mDoctorID,mConsultType,1,mMoney);
    }
    private void addDoctorSchedule(List<HttpUserConsults.ImageFile> imageFiles, final int type) {
        HttpUserOPDRegisters.Add addOPDReg = new HttpUserOPDRegisters.Add(
                id,
                mDoctorID,
                mTimeInfo.mDoctorScheduleID,
                mTimeInfo.mDateStr,
                mOPDType,
                null,
                mConsultContent.getText().toString(),
                imageFiles,
                new HttpListener<UserOPDRegisters.AddResp>() {
                    @Override
                    public void onError(int code, String msg) {
                        if (DebugUtils.debug) {
                            Log.d(TAG, DebugUtils.errorFormat("addOPDReg", code, msg));
                        }
                        ToastMananger.showToast(BuyVVConsultActivity.this, msg, Toast.LENGTH_SHORT);
                        dismissCreateConsultDialog();
                    }

                    @Override
                    public void onSuccess(UserOPDRegisters.AddResp o) {
                        if (DebugUtils.debug) {
                            Log.d(TAG, DebugUtils.successFormat("addOPDReg", PUtils.toJson(o)));
                        }
                        dismissCreateConsultDialog();

                        if (TextUtils.isEmpty(o.mOrderNO) || o.mOrderState == 1) {
                            if (o.mActionStatus.equals("Success")){
                                PUtils.goToMyService(BuyVVConsultActivity.this, 1, EventApi.MainTabSelect.MY_COSULT, false);
                                ToastMananger.showToast(BuyVVConsultActivity.this, o.mErrorInfo, Toast.LENGTH_SHORT);
                        ...
                        PayActivity.startPayActivity(BuyVVConsultActivity.this, o.mOrderNO,"0",type,PayActivity.BUY_VOICE_VIDEO, true, true);
                        EventBus.getDefault().post(new EventApi.BuyVVConsultSuc());
                        finish();
//                        CommonUtils.startActivity(DoctorServiceActivity.this, PayActivity.class);
                    }
                }
        );

        HttpClient addOPDRegClient = NetService.createClient(this, addOPDReg);
        addOPDRegClient.start();

    }

    private void getUserPackages(int i){
        MemberPackages.CheckUserPackage checkUserPackage = new MemberPackages.CheckUserPackage(mDoctorID, i, new HttpListener<CheckUserPackageBean>() {
            @Override
            public void onError(int code, String msg) {
                ToastUtils.showShort(BuyVVConsultActivity.this, getString(R.string.check_user_package_fail));
            }
        ...
        new HttpClient(BuyVVConsultActivity.this, checkUserPackage).start();
    }
