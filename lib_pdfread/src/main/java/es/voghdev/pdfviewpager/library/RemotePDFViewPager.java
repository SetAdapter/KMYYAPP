...
public class RemotePDFViewPager extends ViewPager implements DownloadFile.Listener {
    ...
    public RemotePDFViewPager(Context context, String pdfUrl,String fileName, DownloadFile.Listener listener,String[] heads) {
        super(context);
        this.context = context;
        this.listener = listener;
        this.headers = heads;
        init(pdfUrl,fileName);
    }
    public RemotePDFViewPager(Context context, String pdfUrl,DownloadFile.Listener listener,String[] heads) {
        super(context);
        this.context = context;
        this.listener = listener;
        this.headers = heads;
    ...
    private void init(String pdfUrl,String fileName) {
        DownloadFile downloadFile = new DownloadFileUrlConnectionImpl(context, new Handler(), this);
        downloadFile.download(this.headers,pdfUrl,
                new File(context.getCacheDir(), fileName).getAbsolutePath());
    }
